...
class skype {
    notify(chat_id, project_name, release_page, version_info, cb) {
        ...
        this._requestToken(appId, appSecret, function (err, token) {
            ...
            that._sendMsg(appId, appSecret, token, chat_id, msg, cb)
        })
    ...
    _sendMsg(appId, appSecret, accessToken, chat_id, msg, cb) {
        const options = {
        ...
        request(options, (err, res) => {
            ...
            if (typeof res === 'undefined')
                cb(new Error(`Timeout Error. res object is ${res}`))
            if (res.statusCode !== 201)
                cb(new Error(`res code is ${res.statusCode}, body is ${res.body}`))
